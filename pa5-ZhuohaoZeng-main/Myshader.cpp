...
bool MainShader::setContext(const GMatrix& ctm){
...
void MainShader::shadeRow(int x, int y, int count, GPixel row[]){
    int ix, iy;
...
std::shared_ptr<GShader> GCreateBitmapShader(const GBitmap& bm, const GMatrix& localMatrix, GTileMode mode){
    return std::unique_ptr<GShader>(new MainShader(bm, localMatrix, mode));
};
